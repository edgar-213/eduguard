...
async function renderSection(section) {
    ...
    switch(section) {
        ...
        case 'chat':
            ...
            break;
        case 'groups':
            await Promise.all([loadGroups(), loadProfessorChats()]);
...
function renderGroups() {
    return `
        <div class="content-section active">
            <!-- Tabs para Grupos y Chat con Profesores -->
            <div class="chat-tabs">
                <button class="chat-tab active" onclick="switchChatTab('grupos')">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <button class="chat-tab" onclick="switchChatTab('profesores')">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <!-- Panel de Grupos -->
            <div id="gruposPanel" class="chat-panel active">
            ...
            <!-- Panel de Chat con Profesores -->
            <div id="profesoresPanel" class="chat-panel">
                <div class="groups-header">
                    <h3 style="font-size: 1.5rem; font-weight: 700;">Chat con Profesores</h3>
                </div>
                
                <div class="grid">
                    ${(state.professorChats || []).length > 0 ? state.professorChats.map(chat => `
                        <div class="group-card" onclick="openProfessorChat(${chat.profesor_id})">
                            <div class="group-content">
                                <div class="professor-avatar">
                                    ${chat.profesor_nombre.split(' ').map(n => n[0]).join('')}
                                </div>
                                <div class="group-info">
                                    <div class="group-name">${chat.profesor_nombre}</div>
                                    <div class="group-meta">${chat.materia || 'Profesor'}</div>
                                    <div class="group-last-message">
                                        ${chat.ultimo_mensaje || 'Sin mensajes'}
                                    </div>
                                    ${chat.no_leidos > 0 ? `
                                        <span class="unread-badge">${chat.no_leidos}</span>
                                    ` : ''}
                                </div>
                            </div>
                        </div>
                    `).join('') : '<p>No tienes conversaciones con profesores aún.</p>'}
                </div>
            </div>
        </div>
    `;
...
let currentProfessorId = null; // 👈 AGREGAR ESTA LÍNEA
...
// ========================================
// GRUPOS Y CHAT - VERSIÓN INTEGRADA
// ========================================

// Agregar nueva función para cargar conversaciones con profesores
async function loadProfessorChats() {
    try {
        const response = await fetch(`${API_URL}?action=get_professor_chats`);
        const data = await response.json();
        
        if (data.success) {
            state.professorChats = data.chats || [];
        }
    } catch (error) {
        console.error('Error cargando chats con profesores:', error);
    }
}

// Modificar la función renderGroups para incluir chats con profesores
function renderGroups() {
    return `
        <div class="content-section active">
            <!-- Tabs para Grupos y Chat con Profesores -->
...
function switchChatTab(tab) {
    ...
    if (tab === 'grupos') {
        document.getElementById('gruposPanel').classList.add('active');
    } else if (tab === 'profesores') {
        document.getElementById('profesoresPanel').classList.add('active');
        loadProfessorChats(); // Cargar chats al abrir
    }
}

// Nueva función para abrir chat con profesor
async function openProfessorChat(profesorId) {
    currentProfessorId = profesorId;
    lastMessageId = 0;
    
    const response = await fetch(`${API_URL}?action=get_professor_chat_details&profesor_id=${profesorId}`);
    const data = await response.json();
    
    if (!data.success) {
        showNotification('Error al cargar chat', 'error');
        return;
    }
    
    const mainContent = document.getElementById('mainContent');
    mainContent.innerHTML = renderProfessorChat(data.profesor);
    
    // Cargar mensajes
    await loadProfessorMessages(profesorId);
    
    // Iniciar polling
    if (messagePollingInterval) clearInterval(messagePollingInterval);
    messagePollingInterval = setInterval(() => loadNewProfessorMessages(profesorId), 2000);
    
    initializeProfessorChatListeners();
}

// Renderizar interfaz de chat con profesor
// Renderizar interfaz de chat con profesor (CON EMOJIS E IMÁGENES)
function renderProfessorChat(profesor) {
    return `
        <div class="group-chat-container">
            <div class="group-chat-header">
                <button class="back-btn" id="backToGroups">
            ...
            <div class="group-chat-messages" id="professorChatMessages">
                <div class="loading-messages">Cargando mensajes...</div>
            </div>
            
            <!-- Selector de Emojis -->
            <div class="emoji-picker" id="emojiPickerProf" style="display: none;">
                <div class="emoji-picker-header">
                    <span>Selecciona un emoji</span>
                    <button class="emoji-close" onclick="toggleProfessorEmojiPicker()">×</button>
                </div>
                <div class="emoji-grid">
                    ${EMOJIS.map(emoji => `
                        <button class="emoji-item" onclick="insertProfessorEmoji('${emoji}')">${emoji}</button>
                    `).join('')}
                </div>
            </div>
            
            <div class="group-chat-input">
                <button class="attach-btn" id="uploadProfessorImageBtn" title="Subir imagen">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                        <circle cx="8.5" cy="8.5" r="1.5"></circle>
                        <polyline points="21 15 16 10 5 21"></polyline>
                ...
                <button class="emoji-btn" id="emojiProfBtn" title="Emojis">
                    <span style="font-size: 20px;">😊</span>
                </button>
                <input type="text" id="professorMessageInput" placeholder="Escribe tu mensaje..." />
                <button class="send-btn" id="sendProfessorMessage">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <line x1="22" y1="2" x2="11" y2="13"></line>
                        <polygon points="22 2 15 22 11 13 2 9 22 2"></polygon>
                    </svg>
...
// Cargar mensajes con profesor
async function loadProfessorMessages(profesorId, maintainScroll = false) {
    const container = document.getElementById('professorChatMessages');
    if (!container) return;
    
    const response = await fetch(`${API_URL}?action=get_professor_messages&profesor_id=${profesorId}`);
    const data = await response.json();
    ...
    if (data.success) {
        ...
        updateProfessorMessages(mensajes);
...
// Cargar solo mensajes nuevos
async function loadNewProfessorMessages(profesorId) {
    const container = document.getElementById('professorChatMessages');
    if (!container) return;
    
    const response = await fetch(`${API_URL}?action=get_professor_messages&profesor_id=${profesorId}&after_id=${lastMessageId}`);
    const data = await response.json();
    ...
    if (data.success && data.mensajes.length > 0) {
        ...
        nuevosMensajes.forEach(msg => {
            const messageHTML = renderSingleProfessorMessage(msg);
            container.insertAdjacentHTML('beforeend', messageHTML);
        });
...
function renderSingleProfessorMessage(msg) {
    const isOwn = msg.remitente === 'alumno';
    const time = msg.hora || '';
    
    return `
        <div class="group-message ${isOwn ? 'own' : ''}" data-msg-id="${msg.id}">
            <div class="message-header">
                <strong>${isOwn ? 'Tú' : msg.profesor_nombre}</strong>
                <span class="message-time">${time}</span>
            </div>
            <p class="message-text">${escapeHtml(msg.mensaje)}</p>
        </div>
...
function updateProfessorMessages(mensajes) {
    const container = document.getElementById('professorChatMessages');
    if (!container) return;
    
    if (mensajes.length === 0) {
        container.innerHTML = '<div class="no-messages">No hay mensajes aún. ¡Escribe el primero!</div>';
        return;
    }
    
    container.innerHTML = mensajes.map(msg => renderSingleProfessorMessage(msg)).join('');
}

// Enviar mensaje al profesor
async function sendProfessorMessage() {
    const input = document.getElementById('professorMessageInput');
    if (!input) return;
    ...
    const result = await fetchAPI('send_professor_message', {
        method: 'POST',
        body: {
            profesor_id: currentProfessorId,
            mensaje: mensaje
        }
    });
    
    if (result && result.success) {
        const response = await fetch(`${API_URL}?action=get_professor_messages&profesor_id=${currentProfessorId}&after_id=${lastMessageId}`);
        const data = await response.json();
        
        if (data.success && data.mensajes.length > 0) {
            const container = document.getElementById('professorChatMessages');
            data.mensajes.forEach(msg => {
                container.insertAdjacentHTML('beforeend', renderSingleProfessorMessage(msg));
                lastMessageId = msg.id;
            });
            container.scrollTop = container.scrollHeight;
        }
...
// ========================================
// CHAT PROFESOR-ALUMNO - FUNCIONES NUEVAS
// ========================================

// Inicializar listeners del chat con profesor
function initializeProfessorChatListeners() {
    const sendBtn = document.getElementById('sendProfessorMessage');
    const input = document.getElementById('professorMessageInput');
    const backBtn = document.getElementById('backToGroups');
    
    if (sendBtn) sendBtn.addEventListener('click', sendProfessorMessage);
